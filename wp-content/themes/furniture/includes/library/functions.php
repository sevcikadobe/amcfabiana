...
function sh_get_categories($arg = false, $by_slug = false, $show_all = true)
{
	...
	$cats = array();
	if( $show_all ) $cats[] = __( 'All Categories', 'furniture' );
...
function sh_get_sidebars($multi = false)
{
	...
	else $data = array('' => __('No Sidebar', 'furniture'));
...
function get_the_breadcrumb()
{
	...
	if ( ! is_home())
	{
		$breadcrumb .= '<li><a href="'.home_url().'">'.__('Home', 'furniture').'</a></li>';
		...
		elseif (is_singular())
		{
			...
			if( isset( $taxonomies['product_type'] ) ) unset( $taxonomies['product_type']);
		...
		elseif(is_day()) $breadcrumb .= '<li><a href="">'.__('Archive for ', 'furniture').get_the_time('F jS, Y').'</a></li>'; /** If daily Archives */
		elseif(is_month()) $breadcrumb .= '<li><a href="' .get_month_link(get_the_time('Y'), get_the_time('m')) .'">'.__('Archive for ', 'furniture').get_the_time('F, Y').'</a></li>'; /** If montly Archives */
		elseif(is_year()) $breadcrumb .= '<li><a href="'.get_year_link(get_the_time('Y')).'">'.__('Archive for ', 'furniture').get_the_time('Y').'</a></li>'; /** If year Archives */
		elseif(is_author()) $breadcrumb .= '<li><a href="'. esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) .'">'.__('Archive for ', 'furniture').get_the_author().'</a></li>'; /** If author Archives */
		elseif(is_search()) $breadcrumb .= '<li>'.__('Search Results for ', 'furniture').get_search_query().'</li>'; /** if search template */
		elseif(is_404()) $breadcrumb .= '<li>'.__('404 - Not Found', 'furniture').'</li>'; /** if search template */
...
function sh_register_user( $data )
{
	...
	$message = '<div class="alert-error" style="margin-bottom:10px;padding:10px"><h5>'.__('You must agreed the policy', 'furniture').'</h5></div>';;
	if( !$policy ) $message = '';
	if ( !$user_id && email_exists($user_email) == false ) {
		if( $policy ){
			...
			else $message = '<div class="alert-success" style="margin-bottom:10px;padding:10px"><h5>'.__('Registration Successful - An email is sent', 'furniture').'</h5></div>';
		}
		
	} else {
		$message .= '<div class="alert-error" style="margin-bottom:10px;padding:10px"><h5>'.__('Username or email already exists.  Password inherited.', 'furniture').'</h5></div>';
	}
	return $message;
}
		...
		<div class="media commentBox<?php if( $comment->comment_parent ) echo ' innerComment'; ?>">
			...
			<div class="media-body">
				...
				<h6><?php echo get_the_date('M d, Y');?>  <span>|</span>
					<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => __('Reply', 'furniture')))) ?>   
				</h6>
...
function sh_comment_form( $args = array(), $post_id = null, $review = false )
{
	...
	$fields   =  array(
		'author' => '<div class="col-sm-4"><label for="commenterName">'. __( 'Name *', 'furniture' ).'</label><input id="name" class="form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
		'email'  => '<div class="col-sm-4"><label for="commenterEmail">'. __( 'Email *', 'furniture' ).'</label><input id="email" class="form-control" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div>',
		'url'  => '<div class="col-sm-4"><label for="commenterUrl">'. __( 'Website', 'furniture' ).'</label><input id="url" class="form-control" name="url" ' . ( $html5 ? 'type="text"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30"' . $aria_req . ' /></div>',				
	);
	$required_text = sprintf( ' ' . __('Required fields are marked %s', 'furniture'), '<span class="required">*</span>' );
	/**
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<div class="row m0"><label for="comments">'. __( 'Comment', 'furniture' ).'</label><textarea id="comments" class="form-control" name="comment" cols="45" rows="8" aria-required="true"></textarea></div>',
		'must_log_in'          => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'furniture' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'furniture' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="col-md-12 col-sm-12">' . __( 'Your email address will not be published.', 'furniture' ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="col-md-12 col-sm-12">' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'furniture' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'comments_form',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'leave a comment', 'furniture' ),
		'title_reply_to'       => __( 'Leave a Reply to %s', 'furniture' ),
		'cancel_reply_link'    => __( 'Cancel reply', 'furniture' ),
		'label_submit'         => __( 'post comment', 'furniture' ),
		'format'               => 'xhtml',
	);
	/**
...
{
	$theme_options = get_option( 'furniture'.'_theme_options');
...
function _sh_star_rating( $dis = false )
{
	...
	$titles = array( __('Poor', 'furniture'), __('Satisfactory', 'furniture'), __('Good', 'furniture'), __('Better', 'furniture'), __('Awesome', 'furniture') );
	...
	else
	{
		...
		echo '</div>'."\n";
		printf(__('Average Rating %s', 'furniture'), $evg );
	}
}
...
function _sh_preloader($options)
{
	...
	if( sh_set( $options, 'preloader' ) ): ?>
		<div class="animationload">
			<div class="loader"><?php esc_html_e('Loading...', 'furniture'); ?></div>
		</div>
	<?php endif;
}
